...
import tiktoken
import numpy as np
from mlcore.Block import Block
from mlcore.DataLoaderLite import DataLoaderLite
from mlcore.GPTConfig import GPTConfig
from mlcore.Processing import master_process, device, ddp_world_size, ddp_rank, ddp_local_rank, ddp
...
def main():
    ...
    B = int(64 / 4)  # micro batch size
    T = int(1024 / 4)  # sequence length
    total_batch_size = int(B * T * ddp_world_size)
    ...
    for step in range(max_steps):
        ...
        if step % 50 == 0 or last_step:
            ...
            with torch.no_grad():
                ...
                for _ in range(val_loss_steps):
                    print(f"val_loss_steps: {_}")
                    x, y = val_loader.next_batch()
                    x, y = x.to(device), y.to(device)
                    logits, loss = model(x, y)
                    print(f"val_loss_steps > loss: {loss}")
            ...
            if master_process:
                ...
                if step > 0 and (step % 5000 == 0 or last_step):
                    ...
                    model_path = os.path.join(log_dir, f"model_{step:05d}")
                    checkpoint = {
        ...
        #         print(f"validation loss: {iterate_examples('val')}")
        #         print(f"index : {i}")
        ...
        #         # get the logits
        #         with torch.no_grad():
        #             print(f"torch no_grad: {torch.no_grad()}")
        #             logits, loss = model(tokens)
        #             print(f"torch.no_grad > loss: {loss}")
        ...
        if ((step > 0 and step % 50 == 0) or last_step) and (not use_compile):
            ...
            max_length = 100
            tokens = enc.encode("Hello, hows the US and China relationship?,")
